...
information at `https://data.pycodecov.go.id/DataMKG/TEWS/autogempa.json`.
Read get_latest_earthquake
[reference](reference/api.md/#pycodecov.api.Earthquake.get_latest_earthquake)
for more details.
...
`https://data.pycodecov.go.id/DataMKG/TEWS/gempaterkini.json`.
Read get_strong_earthquake
[reference](reference/api.md/#pycodecov.api.Earthquake.get_strong_earthquake)
for more details.
...
async def main():
    async with Codecov() as pycodecov:
        strong_earthquakes = await pycodecov.earthquake.get_strong_earthquake()
...
information at `https://data.pycodecov.go.id/DataMKG/TEWS/gempadirasakan.json`.
Read get_felt_earthquake
[reference](reference/api.md/#pycodecov.api.Earthquake.get_felt_earthquake)
for more details.
...
async def main():
    async with Codecov() as pycodecov:
        felt_earthquakes = await pycodecov.earthquake.get_felt_earthquake()
...
The link starts with `https://data.pycodecov.go.id/DataMKG/TEWS`
and followed with `/shakemap-filename`. Read get_content
[reference](reference/api.md/#pycodecov.api.Shakemap.get_content)
for more details.
...
starts with `https://data.pycodecov.go.id/DataMKG/MEWS/DigitalForecast`
and followed with `/DigitalForecast-{PROVINCE_NAME}.xml`. Read
get_weather_forecast [reference](reference/api.md/#pycodecov.api.WeatherForecast.get_weather_forecast)
for more details.
...
async def main():
    async with Codecov() as pycodecov:
        weather_forecast_data = await pycodecov.weather_forecast.get_weather_forecast(Province.ACEH)
