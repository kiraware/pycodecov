...
from pycodecov.parsers import parse_humidity_element
...
def test_parse_element_with_invalid_attribute():
    ...
    with pytest.raises(
        CodecovValidationError, match="value tag in timerange tag not found"
    ):
...
def test_parse_element_with_invalid_humidity_text():
    ...
    element.__iter__.return_value = [timerange]
    with pytest.raises(
        CodecovValidationError, match="value tag in timerange tag has no text"
    ):
        for humidity in parse_humidity_element(element):
            pass
